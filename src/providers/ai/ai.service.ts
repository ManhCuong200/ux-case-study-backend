...
export class AiService {
  ...
  async analyzeScreen(imageUrl: string) {
    ...
    try {
      ...
      const listData = (await listResponse.json()) as GeminiListModelsResponse;

      if (!listData.models || listData.models.length === 0) {
        throw new Error(
          `Google returned no models for this Key. Full Response: ${JSON.stringify(listData)}`,
        );
      }
      ...
      if (capableModels.length === 0) {
        throw new Error(
          'No vision-capable models found for this API Key in the current environment.',
        );
      }
